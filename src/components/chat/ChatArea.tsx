...
export function ChatArea({
  ...
  if (!selectedConversation) {
    return (
      <div className="hidden md:flex items-center justify-center h-full w-full text-center bg-gray-50">
  ...
  return (
    <div className={`flex-1 flex flex-col bg-gray-50 h-full min-h-0 ${
      ...
      {/* Chat Header */}
      <div className="sticky top-0 z-10 flex-shrink-0 px-4 sm:px-6 py-3 border-b border-stone-200 bg-white/90 backdrop-blur-lg">
        {selectionMode ? (
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <button onClick={exitSelectionMode} className="p-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-lg transition-colors">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
              <span className="text-lg font-semibold text-gray-900">{selectedMessages.size} selected</span>
            </div>
            <button onClick={handleDeleteSelected} disabled={selectedMessages.size === 0} className="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors disabled:opacity-50">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="flex items-center space-x-3">
            <button onClick={onBackToConversations} className="md:hidden p-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-lg transition-colors">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
              </svg>
            </button>
            <div className="min-w-0 flex-1">
              <h2 className="text-lg font-semibold text-gray-900 truncate">{getOtherPartyDisplayName()}</h2>
              {typingUser ? (
                <p className="text-sm text-brand-600 truncate">typing…</p>
              ) : (
      ...
      <div
        ...
        className="flex-1 overflow-y-auto overflow-x-hidden px-4 sm:px-6 py-4 space-y-1 bg-[#f7f5f2]"
        style={{ minHeight: 0 }}
      >
        {loadingMessages ? (
          <div className="flex items-center justify-center h-full">
            <div className="flex items-center space-x-3">
              ...
              <span className="text-sm text-gray-600">{t('messages.loadingMessages') || 'Loading messages...'}</span>
            </div>
          </div>
        ) : listItems.length > 0 ? (
          listItems.map((item) => {
            if (item.type === 'date') {
              return (
                <div key={item.id} className="sticky top-1 z-[5] flex justify-center py-2">
                  <span className="text-xs font-medium text-gray-600 bg-white/90 backdrop-blur-sm shadow-sm rounded-full px-3 py-1">
                    {item.label}
                  </span>
                </div>
              );
          ...
          <div className="flex items-center justify-center h-full text-center px-4">
            <div className="max-w-sm mx-auto">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-2">{t('messages.startConversation') || 'Start a conversation'}</h3>
              <p className="text-sm text-gray-500">
                Send a message about <span className="font-medium text-brand-600">{selectedConversation.propertyTitle}</span>
              </p>
            </div>
          </div>
        )}
