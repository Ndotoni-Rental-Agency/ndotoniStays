...
const STEPS = [
  {
    ...
    icon: (
      <svg className="h-7 w-7 sm:h-7 sm:w-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
        ...
        <div className="sm:hidden space-y-3">
          {STEPS.map((step) => (
            <div key={step.number} className="flex items-start gap-3 p-3">
              ...
              <div>
                <h3 className="text-sm font-bold text-ink-900">{t(step.titleKey)}</h3>
        ...
        <div className="hidden sm:grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
          {STEPS.map((step, idx) => (
            <div key={step.number} className="relative text-center group">
              ...
              <h3 className="text-lg font-bold text-ink-900">{t(step.titleKey)}</h3>
